...
{
    public abstract class MultiPageProjectTestBase : AbpIntegratedTestBase<MultiPageProjectTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected MultiPageProjectTestBase()
        {
        ...
        protected void UsingDbContext(Action<MultiPageProjectDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MultiPageProjectDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MultiPageProjectDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MultiPageProjectDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MultiPageProjectDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MultiPageProjectDbContext>())
                {
        ...
        protected async System.Threading.Tasks.Task UsingDbContextAsync(int? tenantId, Func<MultiPageProjectDbContext, System.Threading.Tasks.Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MultiPageProjectDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MultiPageProjectDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MultiPageProjectDbContext, Task<T>> func)
        {
            T result;
